...
"""
    kkt_residual_nlp(z, lambda, nu, s, param, res_mat, mu)
...
- `mu::Float64`: the centering parameter.

# Returns
- `Array`: the residual vector. Only returns if argument for res_mat not specified by user.
"""
function kkt_residual_nlp(z, lambda, nu, s, param, res_mat=nothing, mu=nothing)
    n_z = size(z)[1]
    n_lam = size(lambda)[1]
    n_nu = size(nu)[1]
    ...
    if isnothing(mu)
        ...
        res_mat[n_z+1:n_z+n_lam] = Diagonal(s) * lambda - mu * ones((n_lam,))
    end
