...
p1 = plot(t_steps, x1, ylabel="vₜ", ylim=(-2, 5.5), yticks=yticks1, legend=false)
hline!(p1, [0.], color=:red, linewidth=0.5)

yticks2 = ([-2., -1., 0., 1.], ["-2.0", "-1.0", "0.0", "1.0"])
p2 = plot(t_steps, x2, ylabel="wₜ", ylim=(-2, 1), yticks=yticks2, legend=false)
hline!(p2, [0.], color=:red, linewidth=0.5)

yticks3 = ([-1., 0., 1.], ["-1.0", "0.0", "1.0"])
p3 = plot(t_steps, u, ylabel="uₜ", ylim=(-1.5, 1.5), yticks=yticks3, legend=false)
hline!(p3, [0.], color=:red, linewidth=0.5)
hline!(p3, [1.], color=:orange, linewidth=1, linestyle=:dash)
hline!(p3, [-1.], color=:orange, linewidth=1, linestyle=:dash)

p4 = plot(t_steps, du, ylabel="Δuₜ", xlabel="t", ylim=(-1, 1), legend=false)
hline!(p4, [0.], color=:red, linewidth=0.5)
hline!(p4, [0.5], color=:orange, linewidth=1, linestyle=:dash)
hline!(p4, [-0.5], color=:orange, linewidth=1, linestyle=:dash)
